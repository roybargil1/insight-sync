...
export default function ProductSelector() {
  ...
  return (
    <div className="px-4 py-4 border-b border-white/10" ref={ref}>
      ...
      >
        {/* Icon */}
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${selected ? 'bg-white/20' : 'bg-white/10'}`}>
        ...
        <div className="flex-1 min-w-0">
          {selected ? (
            <>
              <p className="text-white font-semibold text-sm leading-tight truncate">{selected.name}</p>
              {selected.company && <p className="text-white/70 text-xs truncate">{selected.company}</p>}
            </>
          ) : (
        ...
        <ChevronDown className={`w-4 h-4 text-white/70 shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
            ...
            <ul className="py-1 max-h-56 overflow-y-auto">
              {products.map(p => {
                ...
                return (
                  <li key={p.id}>
                    <button
                      onClick={() => { setSelectedProductId(p.id); setOpen(false); }}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-accent ${isActive ? 'bg-primary/8' : ''}`}
                    >
                      <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${isActive ? 'bg-primary/15' : 'bg-muted'}`}>
                        <Package className={`w-3.5 h-3.5 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium truncate ${isActive ? 'text-primary' : 'text-foreground'}`}>{p.name}</p>
