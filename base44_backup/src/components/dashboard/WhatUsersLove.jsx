...
export default function WhatUsersLove({ insights }) {
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-muted-foreground ml-1">— Retention Pillars</span>
        </div>
          ...
          {positiveInsights.map((insight) => (
            ...
            >
              <div className="flex items-start gap-2 mb-1">
                ...
                <p className="text-sm font-semibold text-emerald-900 leading-snug line-clamp-2" style={{ wordBreak: 'break-word' }}>
                  {insight.name}
                </p>
              </div>
              {insight.description && (
                <p className="text-xs text-emerald-700 leading-relaxed line-clamp-2">{insight.description}</p>
              )}
              {insight.related_feedback_title && (
                <p className="text-xs text-emerald-600/70 mt-1.5 truncate">
                  via: {insight.related_feedback_title}
                </p>
              )}
            </div>
          ))}
