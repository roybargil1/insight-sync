...
export default function NewestFeatureRequests({ insights }) {
  ...
  const grouped = Object.values(
    insights
      ...
      .reduce((acc, i) => {
        ...
        if (!acc[key]) {
          acc[key] = { ...i, mentionCount: i.occurrence_count || 1 };
        } else {
          acc[key].mentionCount += (i.occurrence_count || 1);
          if (new Date(i.created_date) > new Date(acc[key].created_date)) {
            acc[key] = { ...i, mentionCount: acc[key].mentionCount };
          }
        }
        return acc;
      }, {})
  ).sort((a, b) => b.mentionCount - a.mentionCount || new Date(b.created_date) - new Date(a.created_date));
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-medium text-foreground leading-snug truncate">{insight.name}</p>
