...
export default function InsightCard({ insight, onDelete, selectable, selected, onToggle }) {
  return (
    <Card
      onClick={selectable ? () => onToggle(insight) : undefined}
      className={`p-4 transition-all duration-150 border-2 flex flex-col ${selectable ? 'cursor-pointer' : ''} ${selected ? 'border-primary bg-accent/40' : 'border-border hover:border-primary/30 hover:shadow-md'}`}
    >
      ...
      <div className="flex items-start gap-2 mb-2 min-w-0">
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded-full border font-medium shrink-0 whitespace-nowrap ${categoryColors[insight.category] || 'bg-muted text-muted-foreground border-border'}`}
        >
          {insight.category}
        </span>
      </div>
      ...
      {insight.description && (
        <p
          ...
          style={{ wordBreak: 'break-word', overflowWrap: 'break-word', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
        >
          {insight.description}
        </p>
      )}

      {/* Source type + Occurrence count */}
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <SourceTypeBadge sourceType={getSourceType(insight)} />
        {(insight.occurrence_count || 1) > 0 && (
      ...
      {/* Severity + Feedback link */}
      <div className="mt-auto flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-muted-foreground font-medium">Sev</span>
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map(n => (
              <div key={n} className={`w-2.5 h-2.5 rounded-sm ${n <= insight.severity ? severityColor(insight.severity) : 'bg-border'}`} />
            ))}
          </div>
        ...
        <div className="flex items-center gap-1 min-w-0">
          {insight.related_feedback_title && (
            <span
              ...
              style={{ wordBreak: 'break-word', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '120px' }}
              title={insight.related_feedback_title}
            >
              <Link2 className="w-3 h-3 shrink-0" />
              <span className="truncate">{insight.related_feedback_title}</span>
            </span>
          )}
          {!selectable && onDelete && (
            <Button
