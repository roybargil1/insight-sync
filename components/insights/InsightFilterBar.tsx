...
export function applyInsightFilters(list: Record<string, unknown>[], filters: InsightFilters) {
  let result = [...list];
  ...
  if (filters.dateRange !== "all") {
    ...
    result = result.filter((i) => new Date(i.created_at as string) >= cutoff);
  }
  if (filters.minFrequency > 1) {
    result = result.filter((i) => ((i.occurrence_count as number) || 1) >= filters.minFrequency);
  }
  return result;
}
...
export default function InsightFilterBar({ filters, onChange }: Props) {
  const hasActiveFilters =
    filters.category !== "all" || filters.dateRange !== "all" || filters.minFrequency > 1;

  const reset = () => onChange(defaultInsightFilters);
  const set = (key: keyof InsightFilters, value: string | number) =>
