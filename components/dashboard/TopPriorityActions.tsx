...
const categoryStyle: Record<string, { badge: string; icon: React.ElementType; iconColor: string }> = {
...
export default function TopPriorityActions({ insights }: { insights: Record<string, unknown>[] }) {
  const [expanded, setExpanded] = useState(false);

  const priority = [...insights]
    .filter((i) => i.category !== "Positive" && (i.severity as number) >= 3)
    .sort((a, b) => {
      const occDiff = ((b.occurrence_count as number) || 1) - ((a.occurrence_count as number) || 1);
      if (occDiff !== 0) return occDiff;
      return ((b.severity as number) || 0) - ((a.severity as number) || 0);
    })
    .slice(0, 6);
    ...
    <div>
      <ol className="divide-y divide-border">
        {visible.map((insight, idx) => {
          const cfg = categoryStyle[insight.category as string] || categoryStyle["Feature Request"];
          const Icon = cfg.icon;
          const sev = severityLabel(insight.severity as number);
          return (
            <li key={insight.id as string} className="flex items-start gap-2.5 py-2 first:pt-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-foreground leading-snug">{insight.name as string}</p>
