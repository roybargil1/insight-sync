...
import InsightFilterBar, {
  ...
  applyInsightFilters,
} from "@/components/insights/InsightFilterBar";
...
export default function InsightDashboardPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: (id) => {
      qc.setQueryData(INSIGHTS_QUERY_KEY, (old: Record<string, unknown>[] = []) =>
  ...
  const productInsights = insights.filter(
    (i) => !selectedProductId || i.product_id === selectedProductId
  );
  const sourceModeInsights = productInsights.filter((i) => {
    if (sourceMode === "combined") return true;
    const st = (i.source_type as string) || "Internal";
    if (sourceMode === "internal") return st === "Internal";
    if (sourceMode === "external") return st === "External";
    return true;
  });
  const filteredInsights = applyInsightFilters(
    sourceModeInsights as Record<string, unknown>[],
    insightFilters
  );
  ...
  const sentimentScore =
    filteredFeedback.length > 0
      ? Math.round((positiveCount / filteredFeedback.length) * 100)
      : 0;
  const criticalIssues = filteredInsights.filter(
    (i) => (i.severity as number) >= 5 && i.category !== "Positive"
  ).length;
  ...
  const recentFb = filteredFeedback.filter(
    (f) => f.analyzed && new Date(f.created_at as string) >= d7
  );
  const prevFb = filteredFeedback.filter(
    (f) =>
      f.analyzed &&
      new Date(f.created_at as string) >= d14 &&
      new Date(f.created_at as string) < d7
  );

  const recentSentiment =
    recentFb.length > 0
      ? Math.round(
          (recentFb.filter((f) => f.sentiment === "Positive").length / recentFb.length) * 100
        )
      : null;
  const prevSentiment =
    prevFb.length > 0
      ? Math.round(
          (prevFb.filter((f) => f.sentiment === "Positive").length / prevFb.length) * 100
        )
      : null;
  const sentimentDelta =
    recentSentiment !== null && prevSentiment !== null
      ? recentSentiment - prevSentiment
      : null;

  const recentCritical = filteredInsights.filter(
    (i) =>
      (i.severity as number) >= 5 &&
      i.category !== "Positive" &&
      new Date(i.created_at as string) >= d7
  ).length;
  const prevCritical = filteredInsights.filter(
    (i) =>
      (i.severity as number) >= 5 &&
      i.category !== "Positive" &&
      new Date(i.created_at as string) >= d14 &&
      new Date(i.created_at as string) < d7
  ).length;
  const criticalDelta = recentCritical - prevCritical;

  const TrendBadge = ({ delta, invert = false }: { delta: number | null; invert?: boolean }) => {
    ...
    },
    {
      ...
      cardStyle: {
        borderLeft: `4px solid ${
          sentimentScore >= 70 ? "#10b981" : sentimentScore >= 40 ? "#f59e0b" : "#ef4444"
        }`,
      },
    ...
    return (
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <p className="text-sm text-muted-foreground mb-8">
          AI-extracted insights from your user feedback
        </p>
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-1 p-1 bg-muted/60 rounded-xl w-fit">
        {[
          ...
          { key: "internal" as const, label: "Internal Data" },
          { key: "external" as const, label: "External Pulse" },
        ].map((mode) => (
          <button
            key={mode.key}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {kpis.map(({ label, value, sub, icon: Icon, iconBg, iconColor, valueColor, cardStyle, trend }) => (
          <div
            key={label}
            className="bg-card border border-border rounded-xl p-5 flex items-start gap-4 overflow-hidden"
            style={{
              borderRadius: "12px",
